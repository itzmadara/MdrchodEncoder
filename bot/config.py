...
class Config:
    def __init__(self):
        try:
            ...
            self.APP_ID = config("APP_ID", default="19863702", cast=int)
            self.API_HASH = config(
                "API_HASH", default="6d48cb362a97a43cfc944fd5c0f917f9"
            )
            self.ARIA2_PORT = config("ARIA2_PORT", default=6800, cast=int)
            self.BOT_TOKEN = config("BOT_TOKEN", default="6461620240:AAFqrzBxqlVzjhYLzx-Fby_-uI0qSLz6iIE")
            self.CACHE_DL = config("CACHE_DL", default=False, cast=bool)
            self.CAP_DECO = config("CAP_DECO", default="◉")
            self.C_LINK = config("C_LINK", default="@ongoing_Station")
            self.CMD_SUFFIX = config("CMD_SUFFIX", default=str())
            self.DATABASE_URL = config("DATABASE_URL", default="mongodb+srv://Mrdaxx123:Mrdaxx123@cluster0.q1da65h.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
            self.DBNAME = config("DBNAME", default="Mrdaxx123")
            ...
            self.LOG_CHANNEL = config("LOG_CHANNEL", default="-1002073793218"
            , cast=int)
            ...
            self.OWNER = config("OWNER", default="6885162066")
